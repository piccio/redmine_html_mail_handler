module RedmineHtmlMailHandler
  module MailHandlerPatch
    def self.included(base) # :nodoc:
      ...
      base.class_eval do
        # override ckeditor's one, it's wanted
        alias_method_chain :cleaned_up_text_body, :html
        # override ckeditor's one, its' wanted
        alias_method_chain :extract_keyword!, :html
        alias_method_chain :accept_attachment?, :html
        alias_method_chain :plain_text_body, :html
      end
    end
    ...
    module InstanceMethods
      ...
      def extract_keyword_with_html!(text, attr, format=nil)
        # original keyword extraction from text mail body
        keyword = extract_keyword_without_html!(text, attr, format)
      ...
      def retrieve_images(attachments)
        ...
        inline_attachments.each do |attachment|
          ...
          images["cid:#{cid}"] = file.rich_file.url(:original, timestamp: false)
        end
        images
      end
